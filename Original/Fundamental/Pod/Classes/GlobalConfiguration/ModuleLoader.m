...
- (void)loadModuleWithName:(NSString *)name{
    ...
    //解析Global配置
    NSArray *globals = [environmentElement elementsForName:@"Global"];
    if (globals.count != 1) {
        [Logger logEWithTag:TAG format:@"Global 数量异常 %ld",globals.count];
    }
    GlobalConfiguration *globalConfiguration = [GlobalConfiguration sharedSingleton];
    [self parserElement:globals.firstObject toObject:globalConfiguration];
    ...
    _configurationFilePath = [globalConfiguration.appFilePath stringByAppendingPathComponent:globalConfiguration.moduleConfigurationFileName];
...
- (void)parserElement:(DDXMLElement *)element toObject:(id)object{
    ...
    for (DDXMLElement *item in [element children]) {
        ...
        //只接受4中类型,其他不处理
        if (!([type isEqualToString:@"string"] ||[type isEqualToString:@"bool"]||[type isEqualToString:@"integer"] || [type isEqualToString:@"float"])) {
            continue;
        }
